 ...
 * and writes a `black.svg` where every colour is replaced with #000000.
 *
 * Strategy:
 *  - Replace inline fill/stroke attributes that carry a colour with #000000
...
function toBlack(svgContent) {
  ...
  // 5. Inject a minimal style that forces everything black
  s = s.replace(
    /(<svg[^>]*>)/i,
...
✅  Done!
   ...
   Skipped   : ${skipped}
   Errors    : ${errors}
   Output    : black.svg written into each icon folder
────────────────────────────────
`);
